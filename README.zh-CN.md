...
SwiftUI GUI 的主要控件都补充了 VoiceOver 的标签、数值与提示；Boost 切换、刷新、授权取消与失败的结果会被播报；实时日志区域有明确名称、且不会逐行打断朗读；字体放大时按钮仍可完整阅读。无障碍文案支持 English / 简体中文 / Español。

如实说明：本次改造由明眼开发者编写与自测，没有视障用户参与测试，也未按 WCAG 做过完整审核。因此“自测没有发现问题”并不等于本 App 已经是完全无障碍的。如果使用中遇到阻碍，欢迎反馈，最好附上涉及的操作、VoiceOver 快捷键或系统设置。
