...
- Once the digest matches, open the app: Control-click it and choose **Open**, or remove the quarantine attribute in Terminal:
  ...
  ```
- If you intend to distribute the app publicly, you will need an Apple Developer account, a Developer ID Application certificate and notarization. Until then, treat release builds as personal/testing builds.
- The shell TUI/CLI does not require code signing.
...
The SwiftUI GUI carries VoiceOver labels, values and hints for its main controls, announces the results of Boost changes, refreshes, cancelled authorizations and failures, gives the live log view a name instead of announcing every line, and keeps its buttons readable when the text size grows. The accessibility text is localized in English, 简体中文 and Español.

Honest limit: this pass was written and reviewed by sighted developers. It was not tested by a screen-reader user and has not been audited against WCAG, so “no problems found during review” does not mean the app is fully accessible. If something gets in your way, please report the operation, the VoiceOver keystroke or the system setting involved.
